...
<template>
  <div class="max-w-5xl mx-auto px-4 md:px-8 pt-28 md:pt-32 pb-24">
    ...
    <div class="mb-10 space-y-4">
      <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-zinc-900/80 border border-zinc-800 text-xs font-semibold text-indigo-400">
      ...
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <!-- Total count badge -->
        <div class="flex items-center gap-2 self-start md:self-auto text-xs text-zinc-400 bg-zinc-900/60 border border-zinc-800/80 px-3.5 py-1.5 rounded-full">
    ...
    <div class="sticky top-20 z-30 mb-8 p-3 rounded-2xl bg-zinc-950/80 backdrop-blur-xl border border-zinc-800/70 shadow-2xl shadow-black/60 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-1 md:pb-0">
        <button
          ...
          class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold transition-all duration-200 whitespace-nowrap cursor-pointer"
        ...
        >
          ...
          <span 
            ...
            :class="activeCategory === cat.id ? 'bg-zinc-700 text-zinc-200' : 'bg-zinc-900 text-zinc-500'"
          >
      ...
      <div class="relative w-full md:w-72">
        <span class="material-icons absolute left-3 top-1/2 -translate-y-1/2 !text-base text-zinc-500 pointer-events-none">search</span>
        <input
          v-model="searchQuery"
          type="text"
          placeholder="Cari website, stack, peran..."
          class="w-full bg-zinc-900/90 border border-zinc-800 focus:border-indigo-500/60 rounded-xl pl-9 pr-8 py-2 text-xs text-white placeholder:text-zinc-500 focus:outline-none transition-colors"
        />
        <button 
          v-if="searchQuery" 
          @click="searchQuery = ''"
    ...
    <div class="space-y-4">
      ...
      <div
        ...
        :key="item.id"
        class="group relative p-5 md:p-6 rounded-2xl bg-zinc-900/30 hover:bg-zinc-900/70 border border-zinc-850 hover:border-zinc-700/80 transition-all duration-300 flex flex-col md:flex-row md:items-start justify-between gap-5 cursor-pointer"
      ...
      >
        ...
        <div class="space-y-3 flex-1 min-w-0">
          ...
          <div class="flex flex-wrap items-center gap-2 text-xs">
            <!-- Index indicator -->
            <span class="text-zinc-600 font-mono text-[11px] font-bold">#{{ (index + 1).toString().padStart(2, '0') }}</span>
            ...
            <div v-if="item.source" class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-medium">
          ...
          <div>
            ...
            <p v-if="item.role" class="text-xs text-zinc-400 mt-0.5 font-medium flex items-center gap-1">
          ...
          <div class="flex flex-wrap gap-1.5 pt-2">
            <span
              ...
              :key="tech"
              class="px-2 py-0.5 rounded-md bg-zinc-900 border border-zinc-800/80 text-[10px] font-medium text-zinc-400 group-hover:text-zinc-300 group-hover:border-zinc-700/60 transition-colors"
            >
              {{ tech.trim() }}
            </span>
          </div>
        ...
        <div class="flex md:flex-col items-center md:items-end justify-between md:justify-start gap-2 pt-3 md:pt-0 border-t md:border-t-0 border-zinc-800/60 flex-shrink-0" @click.stop>
          <!-- Live Website External Link -->
          <a
            v-if="item.source"
            :href="item.source"
            target="_blank"
            class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold bg-zinc-800/80 hover:bg-indigo-600 text-zinc-200 hover:text-white border border-zinc-700/70 hover:border-indigo-500 transition-all shadow-sm"
          ...
          <button
            @click="goToDetail(item.slug)"
            class="inline-flex items-center gap-1 px-3.5 py-2 rounded-xl text-xs font-medium text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors cursor-pointer"
          >
    ...
    <div 
      ...
      class="py-20 text-center rounded-3xl bg-zinc-900/20 border border-zinc-800/50 p-8 space-y-4"
    >
      <div class="w-14 h-14 mx-auto rounded-2xl bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-500">
      ...
      <button 
        ...
        class="px-4 py-2 rounded-xl bg-zinc-800 text-zinc-200 hover:text-white text-xs font-semibold hover:bg-zinc-700 transition-colors"
      >
        Reset Filter
      </button>
    </div>
